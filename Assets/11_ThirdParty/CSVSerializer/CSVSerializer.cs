...
public class CSVSerializer
{
    ...
    static public object Deserialize<T>(string text, Type type)
    {
        return CreateArray(type, ParseCSV(text));
    }

    static public object Deserialize(List<string[]> rows, Type type)
    {
        return CreateArray(type, rows);
    }
    ...
    static private object CreateArray(Type type, List<string[]> rows)
    {
        Array array_value = Array.CreateInstance(type, rows.Count - 1);
        Dictionary<string, int> table = new Dictionary<string, int>();
        //FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);  // Include Property
        ...
        {
            fieldNames[i] = fields[i].Name.ToLower();
            fieldInfos.Add(fieldNames[i], fields[i]);
        }
        
        for (int i = 0; i < rows[0].Length; i++)
        {
            table.Add(rows[0][i].ToLower(), i);            
        }                

        for (int i = 1; i < rows.Count; i++)
        {
            object rowdata = Create(rows[i], table, type, fieldNames, fieldInfos);
            array_value.SetValue(rowdata, i - 1);
        }
    ...
    static object Create(string[] cols, Dictionary<string, int> table, Type type, 
        string[] fieldNames, Dictionary<string, FieldInfo> fieldInfos)
    {
        object v = Activator.CreateInstance(type);
        foreach (string n in fieldNames)
        {
            if (table.ContainsKey(n))
            {
                int idx = table[n];
                if (idx < cols.Length)
                    SetValue(v, fieldInfos[n], cols[idx]);
            }
            else
            {
